subroutine write_screen
  ...
  real(kind=8),dimension(:,:),allocatable::Xchem,Xchem_all
#if NENER>0
  ...
  if (krome_chem) then
     allocate(Xchem(1:ncell,nvar-3),Xchem_all(1:ncell,nvar-3))
     Xchem=0; Xchem_all=0
  end if
#if NENER>0
           ...
           do ind=1,twotondim
              ...
              do i=1,ncache
                 if(son(ind_cell(i))==0)then
                    ...
                    if (krome_chem) Xchem(icell,:) = uold(ind_cell(i),4:nvar) / uold(ind_cell(i),1)
#if NENER>0
                    do irad=1,nener
  ...
  call MPI_ALLREDUCE(rr,rr_all,ncell,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
  if (krome_chem) then
     call MPI_ALLREDUCE(Xchem,Xchem_all,ncell*(nvar-3),MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     Xchem = Xchem_all
  end if
  ...
  if(myid==1)then
     write(*,*)'================================================'
     if (krome_chem) then
        write(*,*)'lev      x           d          u          P        Relative abundances'
     else
#if NENER>0
     if(poisson)then
...
116 format(i3,1x,1pe12.5,1x,99(1pe10.3,1x))
